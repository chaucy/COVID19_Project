...
-- 1. 'Fatality rate by country progress over time' 

-- Creating 'CTE' Temp table for rolling sum of 'new_cases' & 'new_deaths'
WITH rolling_sum AS (
    SELECT
        location,
		date,
...
SELECT
    location,
	date,
--CASE statement for avioding dividing zero track back occur
    (CASE 
...
SELECT
	...
	cd.filter_location,
	SUM(ISNULL(CAST(cd.new_deaths_per_million AS float), 0)) OVER (PARTITION BY cd.location ORDER BY cd.date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
